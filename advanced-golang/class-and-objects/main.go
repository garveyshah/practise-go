...
/*An embedded struct holding content from another struct and two other string varables named Animals*/
type Animals struct {
	SuperAnimals
	food   string
	sound string
...
/*
	Now We are indirectly implementing the Animal interface without any keywords.

We are about to define each method decalred in the Animaler interface.
*/
func (x Animals) Eat() {
...
func (x Animals) Move() {
	// this method will acces the variable locomotion in Animal Class
...
// Finally reached main function where we can now test our "GO classes"
func main() {
	/* Experiencing a directory / map in GO. For the animal name as the key, that particular object is a value*/
